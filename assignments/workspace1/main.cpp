...
struct Debugger
{
	...
	float scale = 20.0f;
	float warpStrength = 1.0;
	float specScale = 1.0;
	float brightHigh = 1.0;
	float brightLow = 1.0;
}debug;
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		shader.setInt("water_spec", 1);
		shader.setInt("water_warp", 2);
		shader.setVec3("water_color", debug.waterColor);
		shader.setFloat("scale", debug.scale);
		shader.setFloat("spec_scale",debug.specScale);
		shader.setFloat("warp_strength",debug.warpStrength);
		shader.setFloat("brightness_lower_cutoff",debug.brightLow);
		shader.setFloat("brightness_upper_cutoff",debug.brightHigh);
